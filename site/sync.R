...
for (path in vignettes) {
  ...
  index[[length(index) + 1]] <- data.frame(
    slug = slug, title = title, order = if (is.na(order)) 99L else order,
    stringsAsFactors = FALSE
  )
  message("page  ", slug)
}

nav <- do.call(rbind, index)
nav <- nav[order(nav$order, nav$slug), , drop = FALSE]
writeLines(
  c("// Generated by site/sync.R from the package vignettes. Do not edit.",
    "export const pages = [",
    paste0("  { slug: '", nav$slug, "', title: ", sQuote2 <- paste0("'", gsub("'", "\\\\'", nav$title), "'"), " },"),
    "];"),
  file.path(root, "site", "src", "pages.mjs")
)
