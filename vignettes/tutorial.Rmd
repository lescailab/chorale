...
<p>Two shared biological functions recovered across three cohorts that share no samples, each with a
...
<a href="../input-format">input data</a> page sets this out in full, and
...
The container is a `SummarizedExperiment`, Bioconductor's structure for a feature-by-sample assay
with its sample annotation. The measurements and the sample table are one object from here on, so
they cannot be subset or reordered out of alignment, and a study whose data already sit in a
`SummarizedExperiment` or one of its subclasses passes its `assay()` and `colData()` straight in.
...
compare missing metadata against phenotype.

## Step 3. Give every modality one vocabulary

Candidate functions are compared twice: on what they do to the experiment, and on which biology they
...
fit <- chorale_fit(
  ...
  n_init         = 10,
  n_ambiguity_boot = 19, # short vignette run; production default is 999
  n_pathway_perm = 200,
  seed           = 1
)
fit
...
### Where `n_factors = c(RNA = 3, PROT = 3, METAB = 3)` comes from

`n_factors` is how many latent axes to recover in each modality, and it is the one quantity the
estimator cannot read off its own objective: a decomposition into more axes always fits better, so
the count has to come from outside the fit. Two ways to set it.

**Let parallel analysis choose it.** `n_factors = "auto"` runs
[`chorale_n_factors()`](reference/chorale_n_factors) on each modality, which compares the variance
each component explains against what components explain in data with the same dimensions and no
structure, and keeps those above that threshold:

```r
chorale_n_factors(scale(t(sim$modalities$RNA)))
#> [1] 3
```

Three, which is what the data were simulated with, and what is passed above. On real cohorts this is
the default to start from.

**Set it deliberately.** The count that matters is what the cohort can support, and cohort size is
the binding constraint: seventy samples will not sustain many components, and asking for more
manufactures axes that are noise. Fitting across a range and checking which functions persist is the
practical test. `fit$fits$RNA$stability` reports, for each random restart, the mean correlation
between its axes and those of the selected run, and that agreement falls first where the count is
set too high.

## Step 5. Read the biological functions
...
The column grouping the rows is named `programme`, which is the term the tables and the function
names use for a shared biological function.

Two biological functions survived, each carried by all three modalities. `joint_p` is not the best pairwise
link: it comes from rerunning the entire procedure, assignment included, on shuffled designs, and
keeping the best value any biological function and any set of modalities could have reached by chance. The
smallest value attainable with 200 permutations is 1/201, which is 0.00498, so P3 is at the floor.

Three biological functions were recovered and one was not supported. It is not reported and does not appear
in the tables.
...
pair is significant on its own: the metabolome was diluting a biological function that RNA and protein
carry. For P3, removing any modality lowers or barely changes the statistic and the biological function
...
Neither biological function is corroborated on biology, which is the correct answer here: the simulator plants
...
Each individual was measured once, so how strongly two biological functions move together *within* an
individual is
...
The median width falls from 1.952 to 1.437. That is what conditioning on the design bought. For a
specific pair, `[0.35, 0.70]` would mean every compatible correlation is positive, but the data
cannot select one point. `[-0.95, 0.96]` would leave the sign and strength undetermined. Bound width
comes from missing pairing; a bootstrap sensitivity envelope describes resampling variation.
...
The phenotype permutation keeps the ICA decomposition fixed and repeats phenotype-led assignment
and subset selection after residual permutation. The modality shuffle needs a shared feature space,
which a transcriptome and a lipidome do
...
`evidence` says which lines of support a biological function has.

| Label | Meaning | How much weight it carries |
|---|---|---|
| `design and pathway` | Adjusted phenotype effects agree and pathways corroborate them | Two related evidence channels; both use the fitted factors. |
| `design only` | Phenotype is supported but the pathway channel is not | Check pathway coverage before interpreting disagreement. |
| `pathway only` | Pathways agree without phenotype support | A hypothesis only; it cannot become a resolved phenotype match. |
...
| Matches that contrast missing metadata against phenotype | A covariate holds `""` beside one real level | Check the deposited per-sample design, never the study description |
...
| No biological function supported | The evidence did not beat the null | A real outcome. Report it, and read the per-modality tables |
| Very few factors recovered | Too few samples in that modality | More samples, not more features; reducing feature count does not help |
