...
## 1. Recovering candidate functions inside each modality
...
Each modality is then analysed on its own, using only its own samples. Independent component
analysis separates the measured features into a small number of latent axes, each a direction along
which many features move together. An axis is a candidate biological function: a set of
transcripts, or proteins, or lipids that rise and fall as a unit across the samples in that cohort.
...
## 2. Naming what an axis describes
...
## 3. Asking what each biological function does to the experiment

No individual appears in two modalities, so biological functions cannot be matched by correlating scores. What
...
Two axes in two modalities are candidates for the same biological function when their design profiles point
...
A **shared biological function** is the resulting group: at most one axis per modality, all placed
at the same position. It is a coordinated response the modalities have in common, which need not
correspond to a curated pathway. In the output tables and in the function names the object is
called a programme.

## 5. Judging the biological function, and asking what carries it

A biological function is judged as one object. Its statistic is the agreement of its members' design
profiles, averaged over every pair inside it. Its null is obtained by shuffling the design among
samples and running the entire procedure again, assignment included, keeping the best value any
biological function and any set of modalities could have reached by chance. The p-value therefore accounts
for the freedom the procedure had, rather than conditioning on the biological function it happened to choose.
How many modalities a biological function spans is decided against that same null: the widest set whose joint
evidence survives.

Two further questions are then asked of every surviving biological function.

**Did it need all its modalities?** Each is removed in turn and the biological function rescored without it. A
biological function whose evidence is unchanged was resting on the modalities that remain; one that collapses
...
Knowing that two biological functions behave alike across the design still does not say how they move
together within one individual, and no amount of data of this kind will, because each individual
was measured once.
