...
One row per modality that carries a shared biological function, so one seen in three modalities
occupies three rows. The column grouping those rows is named `programme`, which is the term the
tables and the function names use for the object.

| Column | Meaning |
|---|---|
| `programme` | Identifier of the group. Rows sharing it are one biological function seen in different modalities. |
| `n_modalities`, `modalities` | How far the biological function reaches, and which layers carry it. |
...
| `joint_statistic`, `joint_p` | The biological function judged as one object, and its p-value against the full-procedure null. |
...
| `evidence` | Which channels support the biological function. |

**How to read it.** Start with `evidence`. A biological function labelled *design and pathway* is supported by
two independent lines: the modalities respond to the disease the same way, and they implicate the
same biology, and the second was not built into the first. A biological function labelled *design only* agrees
...
Then read `phenotype_effect` across the rows of one biological function. Signs that agree mean the modalities
...
**One check before believing a small `joint_p`.** A biological function can agree strongly across modalities
while `phenotype_p` is large in every one of them. The modalities are then agreeing on something
other than the disease, most often batch or age, and the biological function describes how the cohorts were
assembled rather than what the disease did. Read the `phenotype_p` column across the rows of a
biological function before reading its `joint_p`.

## leave_one_out.tsv, what each modality contributed

One row per biological function and dropped modality, for biological functions spanning three or more. `delta` is the
...
and the rest do not stand without it. A `delta` near zero or positive means the biological function rested on
the modalities that remain, and the removed one was accompanying rather than contributing. A
biological function is a result of integration only when no single modality can be removed without cost.

## pathway_evidence.tsv, the biological channel

One row per biological function. `pathway_statistic` is the agreement of the members' positions in curated
...
One row per pair of members of a shared biological function, since a correlation concerns two
quantities. One spanning three modalities contributes three rows, all describing the same object.
...
case-control label within stratum, and its value is the p-value of the strongest biological function's joint
...
`factors.tsv` carries every axis recovered, whether or not it entered a biological function. `shared` marks
those in a supported biological function. `n_markers` counts features clearing the purity threshold and
...
A run in which no biological function survives its null reports that, and writes the tables empty. This
