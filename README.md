...
The framework rests on a single result: in unpaired multi-domain linear
latent models the shared latent block is identifiable from marginal
distributions alone, because the modality-specific components are
non-exchangeable and non-Gaussian (Sturma et al., NeurIPS 2023, Theorem
3.1; Timilsina, Lyu & Fu, arXiv:2409.19422, Theorem 1). Identification
fails where those modality-specific structures are made statistically
alike (Sturma et al., Proposition C.1 and Lemma C.2). Modality
heterogeneity is therefore treated as the identifying resource, and
cross-modality harmonisation is excluded.

Two properties hold throughout.

**Interpretability by design.** Every latent factor carries a biological
definition from the moment it is estimated, through pathway-constrained
loadings and a pure-feature requirement, not by post-hoc annotation.
...
generator are implemented; the estimator is scaffolded but not yet
implemented, pending the feasibility gates that determine whether the
linear non-Gaussian identification route applies to the data at hand.
...
- Timilsina S, Lyu Q, Fu X et al. Identifiable Shared Component Analysis
  of Unpaired Multimodal Mixtures. arXiv:2409.19422.
